...
const _assetLink = (recordId) =>
  m(`a[href=/assets/${recordId}]`,
...
//       onclick: function(e) {
//         e.preventDefault()
//         _loadData(recordId, vnode.state)
//       },
//       oncreate: function(e) {
//         e.preventDefault()
//         _loadData(recordId, vnode.state)
...
// const _assetLink = (key) =>
//   m('a',
//     {
//       href: `/assets/${key}`,
//       onclick: function(e) {
//         // e.preventDefault()
//         // vnode.state.recordId = key;
//         // m.redraw()
//       },
//       oncreate: m.route.link, 
...
// const _assetLink = (key) =>
//   m(`a[href=/assets/${key}]`,
//     { oncreate: m.route.link, onupdate: m.route.link, onclick: (e) => {
//       vnode.state.recordId = key;
//       } 
//     },
//     key) 
...
//       oncreate: function(e) {
//         e.preventDefault()
//         // vnode.state.recordId = null
//         // vnode.attrs.key = key
//         // alert(`${key}`)
//         _loadData(key, vnode.state)
//         // alert(`${key}`)
//         m.route(`/#!/assets/${key}`)
//         // then(onsuccess)
//       },
...
const AssetDetail = {
  oninit (vnode) {
    _loadData(vnode.attrs.key, vnode.state)
  ...
  view (vnode) {
    if (!vnode.state.record) {
      return m('.alert-warning', `Loading ${vnode.attrs.key}`)
    }
    ...
    return [
      m('.asset-detail',
        ...
        // m('div',
        //   m(`img[src="${getPropertyValue(record, 'attachments')}"]`)),

        m('p',
          m('img.img-fluid', {src:getPropertyValue(record, 'attachments')})),

        m('p',
          m('div.card',
            m('div.card-body',
            _row(
              _labelProperty('Category', getPropertyValue(record, 'type')),
              _labelProperty('Subcategory', getPropertyValue(record, 'subtype'))),

            _row(
              _labelProperty('Item Id', getPropertyValue(record, 'itemId')),
              _labelProperty('Quantity', getPropertyValue(record, 'quantity')),
              _labelProperty('UoM', getPropertyValue(record, 'uom'))),

            _row(
              _labelProperty('Description', getPropertyValue(record, 'description'))),
        ))),
        ...
        m('p',
          m('div.card',
            m('div.card-body',
            _row(
              _labelProperty('Supplier', getPropertyValue(record, 'supplier')),
              _labelProperty('Ranch', getPropertyValue(record, 'ranch'))),          

            _row(
              _labelProperty('Country of Origin', getPropertyValue(record, 'coo')),
              _labelProperty('Node type', getPropertyValue(record, 'nodeType'))),
              
            _row(
              _labelProperty('Reference Id', getPropertyValue(record, 'referenceId')),
              _labelProperty('Action Date', getPropertyValue(record, 'actionDate'))),          
        ))),
        ...
        m('p',
          m('div.card',
            m('div.card-body',
            _row(
              _labelProperty('Quality info', getPropertyValue(record, 'qualityInfo'))),
        ))),          
        ...
        m('p',
          m('div.card',
            m('div.card-body',
            _row(
              _labelProperty('Dimensions', getPropertyValue(record, 'inventDimensions')),
              _labelProperty('Lot', getPropertyValue(record, 'lot')),
              _labelProperty('Lot Batch', getPropertyValue(record, 'lotBatch'))),
        ))),
              ...
              ? m(ReportValue,
                {
                  ...
                  onsuccess: () => _loadData(vnode.attrs.key, vnode.state)
                })
              : null)),
        ...
        //     m('div.card-body',        
        //     _row(
        //       _labelProperty('Reference Blocks', 
        //         m('a', {href: `/#!/assets/${getPropertyValue(record, 'refBlocks')}`}, `${getPropertyValue(record, 'refBlocks')}`))),
        // ))),  
        ...
        m('p',
          m('div.card',
            m('div.card-body',        
            _row(
              _labelProperty('Reference Blocks', _assetLink(getPropertyValue(record, 'refBlocks')))),
        ))),        
